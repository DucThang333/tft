...
export function BoardBuilder() {
  const { data: boot, loading, error } = usePromiseData(() => tftApi.boardBootstrap(), [])
  const synergies = boot?.synergies ?? []
  const boardChampions = boot?.boardChampions ?? []
  const trayChampions = boot?.trayChampions ?? []
  const boardItems = boot?.boardItems ?? []

  return (
    <ApiStatus loading={loading} error={error}>
        <div className="grid grid-cols-1 md:grid-cols-[280px_1fr_300px] gap-8">
            ...
            <GlassPanel className="w-full p-8 relative overflow-hidden">
              ...
              <div className="flex justify-between mb-8 items-end">
                <div>
                  ...
                  <h1 className="font-headline text-3xl font-bold tracking-tighter">Lưới chiến thuật</h1>
                </div>
